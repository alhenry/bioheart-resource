...
# Load additional cell-level metadata
df_cell_meta = pd.read_csv(CELL_META, index_col=0).rename(columns={"celltype": "scclassify_lv3"})

# Load data
adata = sc.read(H5AD)

# add cell-level metadata to adata.obs
...
metadata = adata.obs.loc[:,["scclassify_lv3", "cpg_id", "n_genes_by_counts", "cohort"]]
...
select_cols = ['index', 'cons', 'sex', 'age', 'n_genes_by_counts']
...
# sample adata
index_sample = df_cov['index'].tolist()
